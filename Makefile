...
tf-resources:
	...
	echo -e "// Package resourcefull generated by Makefile on $(shell date -u -Iseconds). DO NOT EDIT." > "internal/terraform/resource/full/autogen-package.go"
	echo -e "package resourcefull\n\n" >> "internal/terraform/resource/full/autogen-package.go"

	echo -e 'import "github.com/hashicorp/terraform-plugin-framework/resource"\n\n' >> "internal/terraform/resource/full/autogen-package.go"

	echo -e "// GetResources returns all autogenerated resources" >> "internal/terraform/resource/full/autogen-package.go"
	echo -e "func GetResources() []func() resource.Resource {" >> "internal/terraform/resource/full/autogen-package.go"
	echo -e "return []func() resource.Resource{" >> "internal/terraform/resource/full/autogen-package.go"
	grep -r -o -h "func [A-Za-z_]*Resource()" internal/terraform/resource/full/| sort | sed 's/func \(.*\)()/\1,/g' >> "internal/terraform/resource/full/autogen-package.go"
	echo -e "}" >> "internal/terraform/resource/full/autogen-package.go"
	echo -e "}" >> "internal/terraform/resource/full/autogen-package.go"

	gofumpt -l -w "./internal/terraform/resource/full/autogen-package.go"

...
- `hash_policy` (String) Bonding transmit hash policy
    ...
    &emsp;|Format    &emsp;|Description                                                   |
    |------------|----------------------------------------------------------------|
    &emsp;|layer2    &emsp;|use MAC addresses to generate the hash                        |
    &emsp;|layer2+3  &emsp;|combine MAC address and IP address to make hash               |
    &emsp;|layer3+4  &emsp;|combine IP address and port to make hash                      |
    &emsp;|encap2+3  &emsp;|combine encapsulated MAC address and IP address to make hash  |
    &emsp;|encap3+4  &emsp;|combine encapsulated IP address and port to make hash         |
...
- `lacp_rate` (String) Rate in which we will ask our link partner to transmit LACPDU packets
    ...
    &emsp;|Format  &emsp;|Description                                           |
    |----------|--------------------------------------------------------|
    &emsp;|slow    &emsp;|Request partner to transmit LACPDUs every 30 seconds  |
    &emsp;|fast    &emsp;|Request partner to transmit LACPDUs every 1 second    |
...
- `mii_mon_interval` (Number) Specifies the MII link monitoring frequency in milliseconds
    ...
    &emsp;|Format   &emsp;|Description                                    |
    |-----------|-------------------------------------------------|
    &emsp;|0        &emsp;|Disable MII link monitoring                    |
    &emsp;|50-1000  &emsp;|MII link monitoring frequency in milliseconds  |
- `min_links` (Number) Minimum number of member interfaces required up before enabling bond

    &emsp;|Format  &emsp;|Description                                                           |
    |----------|------------------------------------------------------------------------|
    &emsp;|0-16    &emsp;|Minimum number of member interfaces required up before enabling bond  |
- `mirror` (Attributes) Mirror ingress/egress packets (see [below for nested schema](#nestedatt--mirror))
- `mode` (String) Bonding mode

    &emsp;|Format                 &emsp;|Description                                                   |
    |-------------------------|----------------------------------------------------------------|
    &emsp;|802.3ad                &emsp;|IEEE 802.3ad Dynamic link aggregation                         |
    &emsp;|active-backup          &emsp;|Fault tolerant: only one slave in the bond is active          |
    &emsp;|broadcast              &emsp;|Fault tolerant: transmits everything on all slave interfaces  |
    &emsp;|round-robin            &emsp;|Load balance: transmit packets in sequential order            |
    &emsp;|transmit-load-balance  &emsp;|Load balance: adapts based on transmit load and speed         |
    &emsp;|adaptive-load-balance  &emsp;|Load balance: adapts based on transmit and receive plus ARP   |
    &emsp;|xor-hash               &emsp;|Distribute based on MAC address                               |
...
- `interval` (Number) ARP link monitoring interval
    ...
    &emsp;|Format  &emsp;|Description                                                  |
    |----------|---------------------------------------------------------------|
    &emsp;|u32     &emsp;|Specifies the ARP link monitoring frequency in milliseconds  |
- `target` (List of String) IP address used for ARP monitoring

    &emsp;|Format  &emsp;|Description                                                    |
    |----------|-----------------------------------------------------------------|
    &emsp;|ipv4    &emsp;|Specify IPv4 address of ARP requests when interval is enabled  |
...
- `es_df_pref` (Number) Preference value used for designated forwarder (DF) election
    ...
    &emsp;|Format   &emsp;|Description          |
    |-----------|-----------------------|
    &emsp;|1-65535  &emsp;|DF Preference value  |
- `es_id` (String) Ethernet segment identifier

    &emsp;|Format      &emsp;|Description                                 |
    |--------------|----------------------------------------------|
    &emsp;|1-16777215  &emsp;|Local discriminator                         |
    &emsp;|txt         &emsp;|10-byte ID - 00:11:22:33:44:55:AA:BB:CC:DD  |
